...
exports.parseGoDurationSeconds = parseGoDurationSeconds;
exports.translateChaosMeshSpec = translateChaosMeshSpec;
...
function parseGoDurationSeconds(s) {
...
function classifyByKindAndAction(kind, action) {
    switch (kind) {
        case 'NetworkChaos':
...
function translateChaosMeshSpec(crd, nowUnixSeconds) {
    if (!crd.spec.duration) {
        throw new Error(`Chaos Mesh CRD ${crd.kind}/${crd.metadata?.name ?? '?'} ` +
    ...
    const { kind, affected_signals, suppress_families } = classifyByKindAndAction(crd.kind, crd.spec.action);
    return {
        kind,
        affected_signals,
...
//# sourceMappingURL=chaos-mesh-translate.js.map